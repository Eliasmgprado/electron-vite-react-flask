...
current_app.logger.setLevel(current_app.config["LOG_LEVEL"])

# Remove the stdout handler
current_app.logger.removeHandler(current_app.logger.handlers[0])
...
if current_app.config.get("LOG_FILENAME"):
    file_handler = RotatingFileHandler(
        filename=current_app.config["LOG_FILENAME"],
        maxBytes=current_app.config["LOG_MAXBYTES"],
        backupCount=current_app.config["LOG_BACKUPS"],
        mode="a",
        encoding="utf-8",
    )
...
logger = wrap_logger(current_app.logger, processors=[add_fields, JSONRenderer(indent=None)])
